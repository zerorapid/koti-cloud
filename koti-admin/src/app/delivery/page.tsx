...
import { RIDERS, ORDERS } from "@/lib/koti-db";
...
export default function DeliveryPage() {
  ...
  const deliveries = useMemo(() => {
    return ORDERS.map(order => ({
      ...
      const matchesSearch = d.id.toLowerCase().includes(searchTerm.toLowerCase()) || d.customer.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  useEffect(() => {
    let interval: any;
    if (selectedDelivery && selectedDelivery.status === 'Processing') {
      interval = setInterval(() => {
        setSimulationProgress(prev => Math.min(1, prev + 0.002));
      }, 1000);
    }
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <StatCard label="Active In Transit" value={deliveries.filter(d => d.status === 'Processing').length} trend="+12%" icon={Truck} color="text-primary" />
         <StatCard label="Completed Today" value={deliveries.filter(d => d.status === 'Delivered').length} trend="Optimal" icon={CheckCircle2} color="text-green-500" />
            ...
            {deliveries.map((d) => (
              <tr key={d.id} className="hover:bg-secondary/50 transition-all group cursor-pointer" onClick={() => { setSelectedDelivery(d); setSimulationProgress(d.progress); }}>
                <td className="px-8 py-6">
                  <span className="text-sm font-black text-primary">#{d.id}</span>
                </td>
                <td className="px-8 py-6">
                   <p className="text-[13px] font-black text-foreground leading-tight">{d.customer}</p>
                ...
                <td className="px-8 py-6 text-center">
                   <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${
                      d.status === 'Processing' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                      d.status === 'Delivered' ? 'bg-green-50 text-green-600 border-green-100' :
                      'bg-muted text-muted-foreground border-border'
                   }`}>
                      {d.status === 'Processing' && <div className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse" />}
                      {d.status === 'Processing' ? 'In Transit' : d.status}
                   </div>
                </td>
                <td className="px-8 py-6 text-center">
                   <span className="text-[13px] font-black text-primary font-mono">{d.status === 'Processing' ? '14:24' : '--'}</span>
                </td>
          ...
          <div className="relative w-full max-w-6xl bg-background h-full max-h-[850px] shadow-2xl rounded-2xl border border-border animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col lg:flex-row">
             ...
             <div className="flex-1 p-10 overflow-y-auto custom-scrollbar flex flex-col bg-background">
                ...
                <div className="grid grid-cols-2 gap-6 mb-10">
                   <DetailItem label="Customer" value={selectedDelivery.customer} />
                   <DetailItem label="Contact" value="+91 98765 43210" />
                   <DetailItem label="Assigned Rider" value={selectedDelivery.rider.name} />
                   <DetailItem label="Live ETA" value={selectedDelivery.status === 'Processing' ? '14 mins' : '--'} highlight />
                </div>
                ...
                <div className="mt-auto flex gap-3">
                   ...
                   <button className="flex-1 py-4 bg-primary text-white rounded-lg font-black text-[11px] uppercase tracking-widest shadow-sm hover:opacity-90 active:scale-95 transition-all">
                      Mark as Delivered
                   </button>
                </div>
             </div>
