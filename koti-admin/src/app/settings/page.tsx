...
export default function SettingsPage() {
  ...
  const [config, setConfig] = useState({
    ...
    serviceFee: SHARED_CONFIG.serviceFee,
    taxPercent: SHARED_CONFIG.gstPercent,
             ...
             <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
                <Card title="App Configuration" subtitle="Basic app identity and operational controls">
                   <div className="grid grid-cols-2 gap-6 mb-6">
                      <FormGroup label="App Name" defaultValue="Koti Stores" />
                      <FormGroup label="Support Phone" defaultValue="+91 98765 43210" />
                      <FormGroup label="Support Email" defaultValue="support@koti.in" />
                      <FormGroup label="Default Language" type="select" options={['English', 'Hindi', 'Telugu', 'Marathi']} />
           ...
           {activeTab === 'delivery' && (
             <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
                <Card title="Delivery & Rider Configuration" subtitle="SLA, pricing, payout rules, and operational hours">
                   <div className="grid grid-cols-2 gap-6 mb-6">
                      <FormGroup label="Delivery Radius (km)" type="number" defaultValue="5" />
                      <FormGroup label="Promised Delivery Time (mins)" type="number" defaultValue="15" />
                      <FormGroup label="Base Delivery Fee (₹)" type="number" defaultValue="19" />
                      <FormGroup label="Free Delivery Threshold (₹)" type="number" defaultValue="299" />
                      <FormGroup label="Rider Payout per Order (₹)" type="number" defaultValue="22" />
                      <FormGroup label="Packaging Fee (₹)" type="number" defaultValue="5" />
                   </div>
                   <div className="grid grid-cols-2 gap-6 mb-6">
                      <FormGroup label="Surge Pricing Multiplier (Peak)" type="number" defaultValue="1.4" />
                      <FormGroup label="Tip Sharing Policy" type="select" options={['100% to Rider', '50% Rider / 50% Platform', 'Retained by Platform']} />
                   </div>
                   <ToggleItem label="Allow Customer to Choose Delivery Slot" defaultChecked />
                   <ToggleItem label="Enable Live Rider Tracking" defaultChecked />
                </Card>
                <StickyFooter onSave={() => showToast("Delivery settings saved successfully!")} />
             </div>
           )}
           ...
           {activeTab === 'payments' && (
             <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
                <Card title="Payment & Finance Rules" subtitle="Gateways, settlement, tax, and COD handling">
                   <div className="grid grid-cols-2 gap-6 mb-6">
                      <FormGroup label="Primary Payment Gateway" type="select" options={['Razorpay', 'PhonePe PG', 'PayU', 'Cashfree']} />
                      <FormGroup label="Settlement Cycle" type="select" options={['T+1', 'T+2', 'T+7', 'Instant (UPI)']} />
                      <FormGroup label="GST Rate (%)" type="number" defaultValue="5" />
                      <FormGroup label="Auto-Refund Threshold (₹)" type="number" defaultValue="500" />
                   </div>
                   <div className="space-y-4">
                      <ToggleItem label="Enable UPI Payments" defaultChecked />
                      <ToggleItem label="Enable Cash on Delivery (COD)" defaultChecked />
                      <ToggleItem label="Enable Wallet Payments (Paytm/Mobikwik)" />
                      <ToggleItem label="Auto-Verify UPI Intent Callbacks" defaultChecked />
                   </div>
                </Card>
                <StickyFooter onSave={() => showToast("Payment settings saved successfully!")} />
             </div>
           )}
...
function FormGroup({ label, defaultValue, type = "text", options = [], disabled = false }: any) {
  return (
    <div className="space-y-2">
       <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">{label}</label>
       {type === 'select' ? (
         <select className="w-full px-4 py-3 bg-muted/20 border border-input rounded-lg text-[13px] font-bold outline-none focus:border-primary transition-all appearance-none">
