...
import { USERS as SHARED_USERS } from "@/lib/koti-db";
...
export default function UsersPage() {
  ...
  const filteredAndSortedUsers = useMemo(() => {
    let result = SHARED_USERS.filter(u => {
    ...
    return result;
  }, [searchTerm, tierFilter, sortConfig]);
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
           ...
           <p className="text-muted-foreground font-medium mt-1 text-sm">Managing {SHARED_USERS.length} active customer profiles with real-time loyalty tracking.</p>
        </div>
        <div className="flex items-center gap-4">
           <div className="px-5 py-2 bg-card border border-border rounded-lg shadow-sm flex items-center gap-3">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
              <p className="text-[10px] font-black text-foreground tracking-tight uppercase">Sync Status: Healthy</p>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
         <MetricCard label="Diamond Users" value={SHARED_USERS.filter(u => u.tier === 'Diamond').length} icon={Zap} color="text-violet-500" />
         <MetricCard label="Loyalty Score" value="8.4" icon={Star} color="text-amber-500" />
         <MetricCard label="Avg. Spend" value={`₹${(SHARED_USERS.reduce((sum, u) => sum + (u.totalSpent || 0), 0) / SHARED_USERS.length).toFixed(0)}`} icon={TrendingUp} color="text-primary" />
