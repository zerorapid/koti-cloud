...
import { TICKETS } from "@/lib/koti-db";

// --- MOCK DATA ---
const MOCK_CUSTOMERS: Record<string, any> = {
  "CUST-101": { name: "Arjun Mehta", tier: "Diamond", phone: "+91 98765 43210", orders: 42, ltv: "₹28,400", join: "Aug 12, 2024", location: "Nashik Hub" },
  "CUST-103": { name: "Rahul Verma", tier: "Diamond", phone: "+91 98765 43212", orders: 35, ltv: "₹22,100", join: "Sep 20, 2024", location: "Vijayawada Hub" },
  "CUST-102": { name: "Priya Sharma", tier: "Gold", phone: "+91 98765 43211", orders: 18, ltv: "₹8,200", join: "Jan 05, 2025", location: "Nashik Hub" },
  "CUST-104": { name: "Sneha Patel", tier: "Silver", phone: "+91 98765 43213", orders: 5, ltv: "₹1,800", join: "Mar 10, 2026", location: "Global" }
};
...
export default function SupportConsole() {
  ...
  const [chatHistory, setChatHistory] = useState<Record<string, any[]>>(() => {
     ...
     TICKETS.forEach(t => history[t.id] = t.history || []);
     return history;
  });
  ...
  const customer = MOCK_CUSTOMERS[activeTicket.custId];

  // --- SLA ENGINE ---
  useEffect(() => {
    const timer = setInterval(() => {
      setTickets(prev => prev.map(t => {
        if (t.status === 'open' && t.slaMin > -100) {
          return { ...t, slaMin: t.slaMin - 0.1 };
        }
        return t;
      }));
    }, 6000);
  ...
  const filteredTickets = tickets.filter(t => {
     ...
     if (filter === 'diamond') return MOCK_CUSTOMERS[t.custId].tier === 'Diamond';
     if (filter === 'critical') return t.priority === 'critical';
     if (filter === 'pending') return t.agent === 'Unassigned';
     return true;
  });
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] bg-background animate-in fade-in duration-500">
      ...
      <div className="border border-border rounded-xl mb-6 bg-card shadow-sm overflow-hidden">
         ...
         <div className="grid grid-cols-4 divide-x divide-border bg-muted/5">
            <StatItem label="Open Tickets" value={tickets.length} />
            <StatItem label="Diamond Waiting" value={tickets.filter(t => MOCK_CUSTOMERS[t.custId].tier === 'Diamond').length} color="text-amber-600" />
        ...
        <div className="w-80 flex flex-col bg-card border border-border rounded-xl shadow-sm overflow-hidden">
           ...
           <div className="flex-1 overflow-y-auto divide-y divide-border/50">
              {filteredTickets.map(ticket => {
                 const cust = MOCK_CUSTOMERS[ticket.custId];
                 const isDiamond = cust.tier === 'Diamond';
                 const lastMsg = chatHistory[ticket.id]?.slice(-1)[0]?.text || "No messages yet";
                 return (
                    <div 
                      key={ticket.id}
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    ...
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-1.5">
                       {activeTicket.id} <span className="w-1 h-1 bg-muted-foreground rounded-full"></span> {activeTicket.priority}
                    </p>
                 </div>
              </div>
              <div className="flex items-center gap-2">
                 ...
                 <ActionBtn label="Resolve" icon={CheckCircle2} primary onClick={() => {}} />
              </div>
           </div>

           {/* Chat Messages */}
           <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-secondary/5">
              {chatHistory[activeId]?.map((msg, i) => (
        ...
        <div className="w-[300px] flex flex-col bg-card border border-border rounded-xl shadow-sm overflow-y-auto p-8">
           ...
           <div className="space-y-6">
              <div className="grid gap-4">
                 ...
                 <InfoRow label="Orders (LTV)" value={`${customer.orders} (${customer.ltv})`} />
                 <InfoRow label="Joined" value={customer.join} />
              </div>
              ...
              <div className="p-4 bg-muted/10 rounded-xl border border-border">
                 ...
                 <p className={`text-2xl font-black tracking-tighter leading-none ${activeTicket.slaMin < 5 ? 'text-red-500 animate-pulse' : 'text-foreground'}`}>
                    {activeTicket.slaMin > 0 ? `${Math.ceil(activeTicket.slaMin)}m` : 'BREACHED'}
                 </p>
              </div>
